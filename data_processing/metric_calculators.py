...
def _calcular_dias_activos_totales(df_combined):
    ...
    if active_df.empty:
        logging.warning("No hay filas consideradas activas para conteo de días.")
        return results
    ...
    if 'Campaign' in active_df.columns:
        try:
            ...
            logging.info("Días activos calc. %s campañas.", len(days_camp))
    ...
    if 'AdSet' in active_df.columns and 'Campaign' in active_df.columns :
        try:
            ...
            logging.info("Días activos calc. %s AdSets (Campaign/AdSet).", len(results['AdSet']))
    ...
    if 'Anuncio' in active_df.columns and 'Campaign' in active_df.columns and 'AdSet' in active_df.columns:
       try:
           ...
           logging.info("Días activos calc. %s Anuncios.", len(days_ad))
