    ...
    try:
        ...
        if ext in ['.xlsx','.xls']:
            ...
            try:
                ...
                logging.warning("Peek Excel header falló (%s), reintentando con skiprows=[1]", e_peek_excel)
                try:
                     df_peek=pd.read_excel(file_path,engine=engine,dtype=str,nrows=0, skiprows=[1])
                except Exception as e_peek_excel_skip:
                     logging.warning("Peek Excel header con skiprows=[1] también falló (%s)", e_peek_excel_skip)
                     return None
        elif ext=='.csv':
            ...
            try:
                df_peek=pd.read_csv(file_path,dtype=str,sep=sep_f,engine='python',encoding=enc_f,nrows=0)
                 ...
                 logging.warning("Peek CSV header falló, reintentando con skiprows=[1]")
                 try:
                     df_peek=pd.read_csv(file_path,dtype=str,sep=sep_f,engine='python',encoding=enc_f,nrows=0,skiprows=[1])
                      ...
                      logging.warning("Peek CSV header con skiprows=[1] también falló (%s)", e_peek_csv_skip)
                      return None
            except Exception as e_peek_csv:
                 logging.warning("Error inesperado al leer header CSV: %s", e_peek_csv)
                 return None
        ...
        logging.warning("Error general detectando col fecha (%s): %s", os.path.basename(file_path), e)
        return None
        ...
        if ext in ['.xlsx', '.xls']:
            ...
            try:
                dates_series = pd.read_excel(file_path, engine=engine, usecols=[date_column_name], skiprows=skip_rows, dtype=str)[date_column_name]
                 ...
                 logging.warning("Leyendo fechas Excel con skiprows=%s falló (%s) en %s. Reintentando sin skiprows...", skip_rows, e, os.path.basename(file_path))
                 try:
                     dates_series = pd.read_excel(file_path, engine=engine, usecols=[date_column_name], dtype=str)[date_column_name]
                      ...
                      logging.warning("Leer fechas Excel sin skiprows también falló (%s)", e_noskip)
            ...
            if not lines or not any(ln.strip() for ln in lines):
                 logging.warning("CSV vacío o ilegible para detectar separador/encoding: %s", os.path.basename(file_path))
                 return pd.Series(dtype='datetime64[ns]')
            ...
            if not content_line_for_sniffer:
                 logging.warning("No se encontró línea con contenido en CSV para sniffer: %s", os.path.basename(file_path))
                 return pd.Series(dtype='datetime64[ns]')
            ...
            try:
                ...
                logging.warning("Sniffer CSV falló en %s, usando separador estimado: '%s'", os.path.basename(file_path), sep)
            ...
            try:
                dates_series = pd.read_csv(file_path, usecols=[date_column_name], sep=sep, engine='python', encoding=enc, on_bad_lines='skip', dtype=str, skiprows=skip_rows)[date_column_name]
                 ...
                 logging.warning("Leyendo fechas CSV con skiprows=%s falló (%s) en %s. Reintentando sin skiprows...", skip_rows, e, os.path.basename(file_path))
                 try:
                     ...
                     logging.warning("Leer fechas CSV sin skiprows también falló (%s)", e_noskip_csv)
                     return pd.Series(dtype='datetime64[ns]')
        ...
        if dates_series is not None:
            ...
            if parsed_dates.isnull().all() and not dates_series.isnull().all():
                ...
                for fmt_date in common_formats:
                    ...
                    if not parsed_dates_fmt.isnull().all():
                        ...
                        logging.info("Fechas parseadas con formato '%s' para %s", fmt_date, os.path.basename(file_path))
                        break
            ...
            if parsed_dates.isnull().all() and not dates_series.isnull().all() and date_parse is not None:
                 try:
                      ...
                      if not temp_parsed_dates.isnull().all():
                          ...
                          logging.info("Fechas parseadas con dateutil.parser para %s", os.path.basename(file_path))
                 except Exception as e_infer:
                      logging.warning("dateutil.parser falló (%s) para %s", e_infer, os.path.basename(file_path))
            ...
            if parsed_dates.notnull().any():
                ...
                if original_parsed_count > 0 and filtered_count < original_parsed_count:
                    logging.warning("%s fechas filtradas por estar fuera del rango (%s-%s) en %s", original_parsed_count - filtered_count, min_year, max_year, os.path.basename(file_path))
                if parsed_dates.isnull().all() and not dates_series.isnull().all() and original_parsed_count > 0:
                    logging.warning("Todas las fechas parseadas para %s están fuera del rango esperado (%s-%s)", os.path.basename(file_path), min_year, max_year)

            return parsed_dates
        return pd.Series(dtype='datetime64[ns]')
    except Exception as e:
        logging.error("Error general leyendo fechas %s col '%s': %s", os.path.basename(file_path), date_column_name, e)
        traceback.print_exc()
        return pd.Series(dtype='datetime64[ns]')
