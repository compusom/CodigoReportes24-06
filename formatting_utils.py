...
def _format_dataframe_to_markdown(df, title, log_func, float_cols_fmt={}, int_cols=[], pct_cols_fmt={}, currency_cols={}, stability_cols=[], default_prec=2, max_col_width=None, numeric_cols_for_alignment=[]):
    ...
    for col in headers:
        if col in cols_fmt:
            ...
            try:
                ...
                logging.warning(
                    "Error applying format '%s' to column '%s': %s. Column might have unexpected data types.",
                    fmt,
                    col,
                    e_apply_fmt,
                )
    ...
    for col in headers:
        try:
            ...
            logging.error(
                "Error in final str conversion for column '%s': %s. Attempting list comprehension fallback.",
                col,
                e_final_str,
            )
            try:
                ...
                logging.error(
                    "List comprehension str conversion failed for '%s': %s. Column might be corrupted.",
                    col,
                    e_list_comp,
                )
    ...
    for clean_h in clean_headers:
        ...
        if orig_h in df_formatted.columns:
            try:
                ...
                logging.warning(
                    "Error calculating string width for column '%s': %s object has no attribute 'str'. Using header length.",
                    orig_h,
